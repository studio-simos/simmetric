...
All communication with the main Simmetric Chat server goes through the internal widget API (`/api/internal/widget`), authenticated with an API key. The widget service is **not** a React app — it uses Preact for a smaller bundle footprint inside the sandboxed iframe, and the loader is served at `/widget/:widgetId.js` for drop-in embedding on external sites.
...
| Server `hourlyRemaining` | Per session | 20 | 1 hour | DB counter (checked in chat route before SSE) |
| Server `dailyRemaining` | Per session | 5 | 24 hours | DB counter |
...
Tests cover HTTP routes (supertest), middleware, Redis integration, and pure helper logic. The Jest config maps `@simmetric-chat/shared` to the monorepo shared package source and uses `@swc/jest` for transformation. Test files live in `src/__tests__/` (`chat.proxy.test.ts`, `chatPanelLogic.test.ts`, `chatPanel.seam.test.ts`, `globToRegex.test.ts`, `loader.test.ts`, `matchUrlPattern.test.ts`, `rateLimit.test.ts`, `rateLimit.daily.test.ts`, `rateLimit.redis.test.ts`, `redisService.test.ts`, `session.test.ts`, `session-route.test.ts`, `sourceCitationSeam.test.ts`, `useWidgetChat.dedup.test.ts`, `useWidgetConfig.test.ts`, `welcomeScreen.seam.test.ts`, `widgetApi.test.ts`, `widgetApp.test.ts`, `widgetEmbedLayout.seam.test.ts`, `widgetI18n.test.ts`, `widgetOpenState.test.ts`) with shared env setup under `src/__tests__/helpers/setupEnv.ts`.
