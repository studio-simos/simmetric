...
- **State**: Three-tier architecture documented in `docs/STATE_MANAGEMENT.md` (`packages/frontend/docs/STATE_MANAGEMENT.md`). TanStack Query hooks in `src/queries/` for REST/CRUD server state (~33 query-hook files). React Context in `src/contexts/` for UI lifecycle/navigation state (ChatContext — workspace/chat nav, PageMetaContext — page metadata, ThemeContext — dark/light theme, EnterpriseModulesContext). Raw `fetchEventSource` + `useState`/`useRef` in `src/hooks/useChat.ts` for SSE streaming (NOT TanStack Query — SSE is an open persistent connection, not request/response).
...
- **Strict parity**: `pnpm i18n:check` iterates all 8 locales and exits 1 on any missing/extra key in ANY locale relative to `en` — de/fr/es/zh/pt cannot lag (no more EN/IT/RU baseline). It is scoped to the `--namespaces=` list in `package.json` (that list is the source of truth — it has grown well past the original set: `chat.palette`, `chat.comparison`, `chat.fallback`, `chat.modelSelector`, `chat.modelCommand`, `chat.capabilities`, `wiki`, `config`, `archives`, `uploads`, `chat.archive`, `chat.attach`, `mcpHelp`, `documents`, `synthesis.rename`, `settings.webSearch`, `widgets`, `setup.wizard`, `workspace`, `synthesis`, `ocr`, `onboarding`, `settings.workspaceAccess`, `settings.subSections.workspaceAccess`, `skills`, `chat.skillsPalette`, `login`, `settings.sso.ldap`, `settings.dlpDocs`). When you add a feature under one of these namespaces, all keys must exist in every locale or the check fails. To add a namespace, append it to that list — the script is the source of truth.
