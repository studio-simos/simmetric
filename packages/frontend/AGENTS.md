...
- **State**: Three-tier architecture documented in `docs/STATE_MANAGEMENT.md` (`packages/frontend/docs/STATE_MANAGEMENT.md`). TanStack Query hooks in `src/queries/` for REST/CRUD server state (27 query-hook files: auth, chats, chatTokens, workspaces, projects, providers, providerPresets, settings, documents, archives, synthesis, marketplace, MCP connections, widgets, OCR jobs/models/defaults/preferences, backups, uploadDrafts, filters, system, templates, SSO, backup logs/destinations). React Context in `src/contexts/` for UI lifecycle/navigation state (ChatContext — workspace/chat nav, PageMetaContext — page metadata, ThemeContext — dark/light theme, EnterpriseModulesContext). Raw `fetchEventSource` + `useState`/`useRef` in `src/hooks/useChat.ts` for SSE streaming (NOT TanStack Query — SSE is an open persistent connection, not request/response).
...
### i18n
- react-i18next with JSON files in `src/i18n/{en,it,ru,de,fr,es,zh,pt}/translation.json` (8 locales as of quick task `260826-n88-02`; `pt` added last).
- **Strict parity**: `pnpm i18n:check` iterates all 8 locales and exits 1 on any missing/extra key in ANY locale relative to `en` — de/fr/es/zh/pt cannot lag (no more EN/IT/RU baseline). It is scoped to the `--namespaces=` list in `package.json` (canonical: `chat.palette, chat.comparison, chat.fallback, chat.modelSelector, chat.modelCommand, chat.capabilities, wiki, config, archives, uploads, chat.archive, mcpHelp, documents, synthesis.rename, settings.webSearch, widgets, setup.wizard`). When you add a feature under one of these namespaces, all keys must exist in every locale or the check fails. To add a namespace, append it to that list — the script is the source of truth.
- `i18n-usage-check.cjs` (same script) fails on `t()` keys absent from `en` — there is an ALLOWLIST for pre-existing debt; do NOT add new entries to it without verifying the key genuinely does not exist in `en`.
- Shared's `widgetLocalesParity.test.ts` pins the widget locale set ↔ frontend `ALL_LANGUAGES` — add/remove a locale in both places.
- Language persisted to localStorage. Hook: `const { t } = useTranslation()` then `t("key.path")`. `t()` supports interpolation `{{variable}}` and pluralization via `count`.
...
- `UpgradePrompt` component renders locked-state card with feature label and upgrade CTA. The `feature` prop is a `string` (widened in Phase 140 — commodity flags like `web_search`/`lead_export`/`widget_analytics` were removed from the `FeatureFlag` union but still render here).
...
- **TanStack Query** (`src/queries/`) for REST/CRUD server state: 27 query-hook files using `useQuery`/`useMutation` with centralized key registry in `queries/keys.ts`. Query client defaults: 30s staleTime, 1 retry (skipped for 401/403/429), no mutation retries.
...
- i18n keys in `common`: `unexpectedError`, `errorBoundaryDescription`, `errorDetails`, `reloadPage` — available in all 8 languages, parity-checked via `pnpm i18n:check`.
...
- i18n keys: `settings.maintenance.title`, `reindexTitle`, `reindexDesc`,
  `reindexSuccess`, `reindexErrors`, `reindexError`. Required in all 8 locales.
...
*This file documents frontend-specific patterns, components, and conventions. For cross-cutting concerns (RBAC, SSE protocol, feature flags, monorepo commands), see the root `AGENTS.md`.*
