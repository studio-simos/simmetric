...
// bundle-barrel-imports: the default `highlight.js` entry registers ALL 193
// language grammars (~1.5 MB of source, the single largest dependency in the
// main bundle). `lib/core` + explicit registration ships only what chat code
// blocks actually use. Unregistered languages degrade gracefully: the
// highlight() callback below falls back to escaped plain code (no breakage).
...
// Aliases for fenced-block info strings that don't literally match a module
// name (markdown-it hands us the raw info string, users type "sh", "html", …).
