...
| `pnpm i18n:check` | Validate translation key parity across all 8 locales (EN baseline) for the `--namespaces=` list in `package.json` (currently 28 namespaces incl. `chat.palette`, `chat.comparison`, `chat.modelSelector`, `wiki`, `config`, `archives`, `documents`, `synthesis`, `ocr`, `widgets`, `setup.wizard`, `skills`, `onboarding`, `login`, `chat.skillsPalette`, `settings.sso.ldap`, `settings.dlpDocs`); `i18n-usage-check.cjs` additionally fails on `t()` keys absent from `en` |

## Key Directories

- `src/components/` — React components (PascalCase, ~143 files), with `chat/`, `sidebar/`, and `ui/` (shadcn primitives) subdirectories. Includes `ChatPanel` (chat orchestrator with citations, ModelSelector, `/model` slash command, and comparison mode), `ChatSidebar` (chat list with rename/delete, mounted by `AppSidebar`; 10s staleTime via `useChats()`), `ModelSelector` / `ModelPalette` (Cmd+K Spotlight-style switcher) / `ModelComparisonView` (side-by-side grid layout), `SettingsPage` (5 deep-linkable tabs: `profile`, `llm`, `appearance`, `security`, `advanced` — each containing permissioned sub-sections; admin sections lazy-loaded via `React.lazy`), `SettingsRoles`, `SettingsProviders`, `SettingsOcr`, `SettingsMaintenance`, `WidgetsPage` (widget admin UI — SettingsPage redirects `tab=widgets` to `/widgets`), `SkillsPage` / `SkillsPalette` (skills management + palette), `LoginPage` / `OnboardingWizard`, DLP panels (`DlpAuditPanel`, `DlpDocumentScanPanel`), `MarketplacePage` / `MarketplaceDetail` / `MarketplaceCard`, archive UI (`ArchivesPage`, `ArchiveDetailPage`, `ArchiveGraphView`, `ArchiveConfigPanel`), synthesis UI, OCR UI, and backup management.
- `src/queries/` — TanStack Query hooks (30 hook modules, 168 exported hooks) for REST/CRUD server state: `useAuth`, `useChats`, `useChatTokens`, `useWorkspaces`, `usePersonalWorkspace`, `useProjects`, `useProviders`, `useProviderPresets`, `useSettings`, `useDocuments`, `useArchives`, `useSynthesis`, `useMarketplace`, `useMcpConnections`, `useWidgets`, `useWidgetWorkspaceArchive`, `useLicense`, `useOcrJobs`, `useOcrModels`, `useDlpPatterns`, `useDlpDocs`, `useBackupDestinations`, `useBackupJobs`, `useBackupLogs`, `useUploadDrafts`, `useFilters`, `useSso`, `useSkills`, `useTemplates`, `useSystem`. Centralized key registry in `queries/keys.ts`; client in `queries/queryClient.ts`.
...
- `src/lib/` — Shared library utilities: `toast.ts` (sonner helpers `showSuccess` / `showError` / `showInfo` / `toastWithAction`), `uiFontScale.ts` and `uiDensity.ts` (FOUC-safe module-init), `synthesisReason.ts` (maps synthesis run errors to localized i18n keys), `utils.ts`.
...
- **TanStack Query** (`src/queries/`, 30 hook modules) — REST/CRUD server state. Query client defaults: 30s staleTime, refetch on window focus, 1 retry (skipped for 401/403/429), no mutation retries. Centralized cache key registry in `queries/keys.ts`.
...
- **Cmd+/ or Ctrl+/** — toggle the navigation overlay.
- **Esc** — close palette or comparison mode.

Registered in `App` via `useKeyboardShortcuts` (listens on `document`, prevents default browser behavior, skips key repeats and Alt-modified keys). The nav-overlay shortcut (`Cmd+/`) was added to avoid conflicts with macOS Cmd+M (minimize) and Chromium Cmd+J (devtools).
