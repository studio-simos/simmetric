...
Express API backend for Simmetric Chat. Handles authentication, RBAC, multi-tenant organization scoping, agent orchestration with SSE streaming, document management, workspace/project scoping, custom skills, DLP document masking, storage providers (LocalFS/S3), webhooks, push notifications, analytics, MCP marketplace, per-chat model management, and the embeddable widget system.
...
- **Plugin seams**: optional peer plugins loaded at boot — `@simmetric-chat/enterprise` via `src/services/enterpriseLoader.ts` (SSO/SCIM/LDAP, audit log, white-label branding, backups) and `@simmetric-chat/saas` via `src/services/saasLoader.ts` (SaaS contract v2 — multi-tenancy, storage providers). Shared loader machinery lives in `src/services/pluginLoaderCore.ts`. Absent = community mode (graceful); broken install = fail-loud `process.exit(1)`. See `docs/ENTERPRISE_PLUGIN.md`.
- **Multi-tenancy (Phase 185)**: every authenticated JWT router carries `tenantContextMiddleware` (auth → tenant → permission); `src/utils/scopedPrisma.ts` adds the tenantScope Prisma extension (defense-in-depth read scoping on direct non-null-org models). `DEFAULT_ORG_ID` covers community installs.
...
| File | Purpose |
|------|---------|
| `src/index.ts` | Main application entry point — exports `createApp()` (used by supertest integration tests) and `mountCatchAlls()`; when run directly it boots: production guards (`ENCRYPTION_KEY` fail-loud, `REDIS_URL` advisory) → `prisma.$connect()` → auto-seed (roles/permissions/menu sections → `initLicense()` → workspace templates → config defaults + setup-wizard-mode derivation → MCP catalog, widget service account + widget API-key row, bootstrap admin → idempotent backfills incl. ArchivePage titles, `localLLMOnly` propagation, `raw/`→`raw_sources/` rename, `searchVectorMulti`) → `initPostgreSQLFTS()` → `initFilters()` → `autoDetectOllama()` → `startJobQueue()` (pg-boss) + `healPgbossQueueDuplicates()` → `loadEnterprisePlugin(app)` → `loadSaaSPlugin(app)` → community LDAP composite route (`/api/auth`) → `mountCatchAlls()` → Ollama model refresh → production-only schedulers (MCP health check, MCP/synthesis/upload-draft/chat-message reapers, vector cleanup, fidelity sampling, MCP connection init, DLP document scan + backfill one-shot queues) → OCR + synthesis pipeline pollers and the wiki-consistency cron (run in dev and production alike) → graceful shutdown (SIGTERM/SIGINT, 5s race; teardown order: SaaS → enterprise → `prisma.$disconnect()`) |
...
src/
  ...
  routes/               # Domain route files (one per resource, 50 files)
  services/             # Business logic and domain services (96 top-level .ts files, incl. synthesis/ + storage/)
  middleware/           # Express middleware (auth, RBAC, rate limit, license, widget CORS, archiveAccess, uploadGate, tenantContext)
  ...
  utils/                # Prisma singleton + tenantScope extension, logger, tenant context, file/archive helpers
  ...
  __tests__/            # Unit and integration tests + helpers/fixtures (293 TS files: 279 test files + helpers/fixtures)
...
prisma/
  ...
  prisma.config.ts      # Prisma 7 config (directory mode, migrations path, datasource URL)
...
- **`routes/`** — HTTP route handlers. One file per domain: `auth.ts` + `authLdapComposite.ts` (community LDAP local-fallback router, Phase 193), `users.ts`, `roles.ts`, `workspaces.ts`, `projects.ts`, `documents.ts`, `chat.ts` + chat splits (`chatCrud.ts`, `chatList.ts`, `chatAgentConfig.ts`, `chatExport.ts`, `chatImport.ts`, `chatRetention.ts`, `chatTokens.ts`), `widgets.ts`, `internalWidget.ts`, `mcp.ts`, `mcpPins.ts`, `marketplace.ts`, `archives.ts` + archive splits (`archiveIndex.ts`, `archivePages.ts`, `archiveSearch.ts`, `archiveGraph.ts`, `archiveConfig.ts`, `archiveExport.ts`, `archiveImport.ts`, `archiveSchemaTemplates.ts`), `synthesis.ts`, `ocr.ts`, `providers.ts`, `providerPresets.ts`, `analytics.ts`, `settings.ts`, `system.ts`, `license.ts`, `templates.ts`, `skills.ts`, `uploads.ts`, `wikilinks.ts`, `wikiChat.ts`, `webhooks.ts`, `push.ts`, `apiKeys.ts`, `health.ts`, `memories.ts`, `filters.ts`, `dlpPatterns.ts`, `e2eHelpers.ts`. SSO (SAML/OIDC), SCIM, and backup routes are mounted by the enterprise plugin (`ctx.mountProtected` / `ctx.mountPublic`) — in a community build those paths 404 (except `POST /api/auth/ldap/login`, which falls through to `authLdapComposite.ts` local auth when the SsoConfig row reports provider `ldap`).
- **`services/`** — Business logic: `authService.ts`, `apiKeyService.ts`, `licenseService.ts`, `webhookService.ts`, `systemConfigService.ts`, `hybridSearchService.ts`, `ftsService.ts`, `encryptionService.ts`, `archiveService.ts`, `synthesisService.ts` + `synthesis/` subdirectory, `ocrJobService.ts`, `providerService.ts`, `agentBudgetService.ts`, `dlpFilter.ts` + the Phase 192 DLP document pipeline (`dlpDocumentMasking.ts`, `dlpDocumentScanJob.ts`, `dlpBackfillService.ts`, `dlpEntityService.ts`, `dlpRecomposeService.ts`, `dlpEvalService.ts`, `dlpChecksum.ts`, `dlpNer.ts`), `skillService.ts` (custom prompt-template skills, Phase 190), `organizationService.ts` (default-org membership, Phase 182), `storageProvider.ts` (LocalFS/S3 strategy) + `storage/`, `pluginLoaderCore.ts` + `enterpriseLoader.ts` + `saasLoader.ts`, `seedService.ts`, `eventLogService.ts`, `templateService.ts`, `redisService.ts`, `distributedLock.ts`, `tokenRevocation.ts`, `jobQueue.ts` (pg-boss singleton), `webSearchService.ts`, `uploadDraftService.ts`, `avatarService.ts`, `ollamaAutoDetectService.ts`, `rerankService.ts`, `postProcessingService.ts`, and more (96 top-level files).
- **`middleware/`** — Cross-cutting concerns: `auth.ts` (JWT/API key + jti revocation), `rbac.ts` (role/permission checks + IDOR prevention), `tenantContext.ts` (multi-tenant resolution: auth → tenant → permission chain; fail-closed 404), `rateLimit.ts` (tiered rate limiting, Redis-backed when available), `license.ts` (feature gating, HTTP 402), `widgetCors.ts` (dynamic CORS for widget embeds), `archiveAccess.ts` (archive-scoped access), `uploadGate.ts` (non-admin upload gating).
- **`agent/`** — Agent orchestration (`orchestrator.ts`), LLM streaming adapters (`llmStreaming/` — per-provider parsers for Ollama/OpenAI/Anthropic/Gemini + reasoning formats; facade at `llmStreaming.ts`), built-in skills (`builtinSkills.ts`), skill registry (`skills.ts`), custom skill executor (`services/skillService.ts` — spotlight-wrapped template compilation), MCP client/server (`mcpClient.ts`, `mcpServer.ts`), implicit tool-call mapping (`implicitToolCall.ts`), tool-call resolver (`toolCallResolver.ts`), model fallback (`modelFallback.ts`), plan parser/runner (`planParser.ts`, `planRunner.ts`), context compaction (`contextCompaction.ts`), citation dedup (`citationDedup.ts`), memory subsystem (`memoryExtraction.ts`, `memoryRetrieval.ts`, `memoryPathRank.ts`, `memorySandbox.ts`, `memoryService.ts`), agent types (`agentTypes.ts`).
- **`utils/`** — `prisma.ts` (Prisma singleton with the `withSoftDelete()` extension) + `scopedPrisma.ts` (tenantScope read-scoping extension), `tenantContext.ts` (AsyncLocalStorage tenant store + `runInTenant`/`bypassTenantScope`), `ssrfGuard.ts` (outbound-request URL screening), `collectorDispatchAgent.ts` (undici-based collector dispatch), `logger.ts`, `auth.ts`, file/archive helpers (`fileUtils.ts`, `archivePath.ts`, `pdfjsFonts.ts`), `slugify.ts`, `deriveTitle.ts`, `fetchDiagnostics.ts`, `parseMetadata.ts`.
...
- **RBAC**: 36 permissions defined in `packages/shared/src/constants/permissions.ts` (workspace, project, chat, document, admin, creation, provider, archive, backup, memory, filters, skills, DLP-unmask groups). 2 default roles seeded: `admin`, `user` (admin acts as superuser). 14 menu sections control sidebar visibility (`dashboard`, `chat`, `documents`, `knowledgeBase`, `workspaces`, `projects`, `marketplace`, `mcpConnections`, `eventLog`, `analytics`, `widget`, `settings`, `uploads`, `skills`) via the `RoleMenuSection` join table.
...
### SSO (SAML / OIDC / LDAP / SCIM) — enterprise plugin

SSO lives in the `@simmetric-chat/enterprise` plugin (mounted via `ctx.mountPublic` / `ctx.mountProtected` in `register(ctx)`): SAML 2.0 SP-initiated login (`/api/auth/saml/*`), OIDC/OAuth with built-in Google/GitHub/Microsoft providers (`/api/auth/oidc/:provider/login`, Phase 165 — env keys override the DB SsoConfig row for infra-as-code deployments), LDAP/Active Directory direct-bind login (`POST /api/auth/ldap/login`, Phase 193 — group→role sync with `assignedVia: "ldap"` revocation semantics), and SCIM 2.0 provisioning (`/scim/v2`, Bearer-token auth via `SCIM_BEARER_TOKEN`). The community build exposes none of these paths except the LDAP composite fallback: `src/routes/authLdapComposite.ts` serves local auth to `POST /api/auth/ldap/login` when the SsoConfig row reports provider `ldap` and the enterprise route defers via `next()` (fallback-eligible arms) — uniform 401 `{ error: "Invalid credentials" }` on every failure arm, no path disclosure. All SSO flows use JIT user provisioning and replay-protection (SAML `InResponseTo` validation, OIDC `nonce` + signed state cookies). See `docs/ENTERPRISE_PLUGIN.md` and `docs/LDAP.md`.
...
- **Feature flags** (`src/middleware/license.ts`): `requireFeature(flag)` gates boolean enterprise features with HTTP 402 `{ error, feature, tier }`; `requireFeatureLimit(flag, model)` gates numeric limits. 12 flags defined in `packages/shared/src/constants/license.ts`: `sso_enabled`, `audit_log_immutable`, `white_label`, `max_workspaces` (Community 3), `max_projects` (Community 3), `custom_agents` (Community 3), `max_skills` (Community 3), `widget_enabled`, `max_widgets` (Community 1), `backup_enabled`, `max_backup_destinations` (Community 1), `widget_credits_editing`. Community defaults are all-off / limited; Enterprise defaults unlock everything (`Infinity` for numeric limits).
...
Beyond the crons, Phase 192 added one-shot pg-boss queues (send/work, production-only) for the DLP document scan + legacy backfill pipelines (`initDlpDocumentScanScheduler()` / `initDlpBackfillScheduler()` — each null-guards `getBoss()` internally; pg-boss offline = scan offline, logged at warn, never blocks boot).

The OCR/URL ingestion pipeline and the synthesis pipeline run as in-process 10-second pollers (with `isRunning` guards and a 2-job concurrency cap for OCR), in dev and production alike. The backup scheduler is registered by the enterprise plugin via the scheduler lifecycle hook. Shutdown drains pg-boss in-flight jobs (4.5s cap) before SaaS + enterprise teardown and `prisma.$disconnect()`, all raced against a 5s hard timeout that force-exits.
...
- **DLP document pipeline (Phase 192)**: whole-document PII masking — admin-triggered scans (`dlpDocumentScanJob.ts`, one-shot pg-boss queues), masked-chunk persistence + collector re-embed (`dlpDocumentMasking.ts` — the original unmasked text is never re-embedded), legacy backfill (`dlpBackfillService.ts`), entity catalog + checksum dedup (`dlpEntityService.ts`, `dlpChecksum.ts` via `stdnum`), chat-stream re-composition for `dlp:unmask`-permitted users (`dlpRecomposeService.ts` — placeholders `[PERSON_1]` → decrypted originals at stream end), and an eval harness (`dlpEvalService.ts`, `dlpNer.ts`). See `docs/DLP_DOCUMENT_PIPELINE.md`.
- **Custom skills (Phase 190)**: user-authored prompt-template skills (`src/services/skillService.ts`) — whitelisted `{{param}}` compilation, spotlight-delimiter injection guard, per-request DB resolution (never registered into the builtinSkills map), owner-or-admin delete, `max_skills` license limit, `/api/skills` CRUD + `/api/agent` invocation. See `docs/CUSTOM_SKILLS.md`.
- **Storage providers (Phase 184)**: per-org storage strategy (`src/services/storageProvider.ts`) — LocalFS default or S3-compatible (`storage/s3Provider.ts`: AWS S3, MinIO, R2, Wasabi via `@aws-sdk/client-s3` + multipart upload), resolved per-request from SystemConfig (`STORAGE_PROVIDER` + 5 `S3_*` keys; fail-loud named errors when S3 config is incomplete — never a silent LocalFS fallback). Document/draft/avatar byte flows cross this interface; `/avatars` + `/branding` serving moved to provider-first download handlers with a permanent fs fallback arm.
...
- `@simmetric-chat/saas` — optional peer dependency (the SaaS plugin, contract v2; installed via `file:../../../simmetric-saas` in dev, air-gap tarball in production). Provides the multi-tenancy + storage-provider surfaces the community build defaults (single `DEFAULT_ORG_ID` org, LocalFS).
...
- `pg-boss` (^12.28.0) — cron job queue + one-shot scan jobs (reapers, health checks, fidelity sampling, wiki consistency, DLP document scans)
- `ioredis` (^5.11.1) + `rate-limit-redis` (^6.0.1) + `redlock` (5.0.0-beta.2) — Redis scale layer (v0.19)
- `@modelcontextprotocol/client` / `core` / `node` / `server` / `server-legacy` (2.0.0) — MCP client/server (SDK 2.0 split packages)
- `jsonwebtoken` (^9.0.3) — JWT auth
- `axios` (^1.19.0) — HTTP client (collector callbacks, LLM APIs, webhooks)
- `undici` (^7.29.1) — HTTP client for the collector dispatch agent
...
- `puppeteer` (^25.9.0) + `@mozilla/readability` (^0.6.0) — URL ingestion / readability
- `pdfjs-dist` (^6.2.108), `sharp` (^0.35.3) — Document parsing and image processing
- `@aws-sdk/client-s3` + `@aws-sdk/lib-storage` (^3.1127.0) — S3-compatible storage provider (multipart upload)
- `graphology` (0.26.0) + `graphology-communities-louvain` (2.0.2) — wiki graph analytics
- `zod` (^4.4.3) — Schema validation
- `bcryptjs` (^3.0.3) — Password hashing
- `stdnum` (^1.12.6) — Entity-checksum validation for DLP checksums
...
| `OLLAMA_CONTAINER_NAME` | Ollama container for the Ollama Cloud `docker exec … ollama login` flow | `simmetric-chat-ollama` |
...
| `AGENT_MAX_TOOL_OUTPUT_LENGTH` | Per-skill output truncation length | `5000` |
...
| `OIDC_ENABLED` / `OIDC_PROVIDER` / `OIDC_DISCOVERY_URL` / `OIDC_CLIENT_ID` / `OIDC_CLIENT_SECRET` / `OIDC_REDIRECT_URI` / `OIDC_SCOPE` | OIDC env overrides of the DB SsoConfig row (Enterprise; air-gap infra-as-code deployments) | — |
| `LDAP_URL` / `LDAP_BIND_DN` / `LDAP_BIND_PASSWORD` / `LDAP_SEARCH_BASE` / `LDAP_SEARCH_FILTER` / `LDAP_GROUP_SEARCH_BASE` / `LDAP_GROUP_SEARCH_FILTER` / `LDAP_USE_TLS` / `LDAP_ACCEPT_CERT` / `LDAP_FALLBACK_TO_LOCAL` | LDAP provider env overrides (Enterprise, Phase 193; env overrides the DB row, no `.min(1)` on the password) | — |
...
| `SEED_ADMIN_USERNAME` / `SEED_ADMIN_PASSWORD` / `SEED_ADMIN_EMAIL` | Bootstrap admin credentials (password min 8 chars; carries `mustChangePassword=true`) | `admin` / `admin123` / `admin@example.com` |
...
- **Strict boundary**: The server only imports from `@simmetric-chat/shared` (plus the two optional plugin packages via the loader seams). It never imports from `collector`, `frontend`, or `widget`. pnpm strictness enforces no phantom deps.
