...
- **ORM**: Prisma 7.10 with PostgreSQL (`@prisma/adapter-pg` + `pg` Pool, singleton at `src/utils/prisma.ts` with `withSoftDelete` extension — never `new PrismaClient()`)
- **Job scheduling**: pg-boss 12.28 cron jobs (MCP health check, reapers, fidelity sampling, wiki consistency) — graceful degradation to offline when Postgres is unavailable; the OCR and synthesis pipelines run as in-process 10-second pollers
...
| File | Purpose |
|------|---------|
| `src/index.ts` | Main application entry point — exports `createApp()` (used by supertest integration tests) and `mountCatchAlls()`; when run directly it boots: `prisma.$connect()` → auto-seed (roles, permissions, menu sections, templates, config defaults, widget API key, bootstrap admin, idempotent backfills) → `initLicense()` → `initPostgreSQLFTS()` → `initFilters()` → `autoDetectOllama()` → `startJobQueue()` (pg-boss) → `loadEnterprisePlugin(app)` → `mountCatchAlls()` → Ollama model refresh → production-only schedulers (MCP health check, MCP/synthesis/upload-draft/chat-message reapers, fidelity sampling, vector cleanup, MCP connection init) → OCR + synthesis pipeline pollers → graceful shutdown (SIGTERM/SIGINT) |
...
| `pnpm build` | Compile TypeScript to `dist/` (via `tsconfig.build.json`) and copy synthesis templates |
...
src/
  ...
  routes/               # Domain route files (one per resource, 48 files)
  services/             # Business logic and domain services (77 files)
  middleware/           # Express middleware (auth, RBAC, rate limit, license, CORS, archiveAccess, uploadGate)
  ...
  templates/            # Synthesis prompt templates (JSON)
  smoke/                # Runtime smoke scripts (ollamaJs.smoke.ts)
  __tests__/            # Unit and integration tests + helpers (201 files)
...
- **IDOR prevention**: `requireProjectAccess` and `requireWorkspaceAccess` middleware verify ownership/access on every relevant route; soft-deletable queries include `where: { deletedAt: null }` via the `withSoftDelete()` Prisma extension. Chat-scoped `archiveId` is threaded deterministically (LLM-passed `metadata.archiveId` ignored for wiki skills) to prevent cross-archive IDOR.
...
- **Skill registry** (`src/agent/skills.ts`): `registerSkill()` / `getSkillsForWorkspace()` / `resolveSkillsForChat()`. Seven built-in skills (`rag_search`, `memory_search`, `web_search`, `workspace_memory`, `document_temp_process`, `wiki_query`, `wiki_write`) registered on module import; MCP skills registered dynamically with `mcp_${connectionName}_${toolName}` namespacing. `web_search` is triple-gated (`ALLOW_WEB_SEARCH` env + `web_search_provider` SystemConfig — the old `web_search` license flag was removed in Phase 140); `memory_search` requires the `memory_enabled` SystemConfig.
...
- **Tool-call resolver** (`src/agent/toolCallResolver.ts`) + **model fallback** (`src/agent/modelFallback.ts`): parse tool calls across the provider formats and cascade per-chat → workspace → global → env with graceful fallback.
- **Hybrid RAG** (`src/services/hybridSearchService.ts`): merges vector store results (LanceDB default, Qdrant, pgvector, or Chroma — resolved via collector HTTP API) with PostgreSQL tsvector full-text search via Reciprocal Rank Fusion with `RRF_K = 60`. Results are tagged `vector`, `fts`, or `both`. Multi-workspace queries use a second-pass RRF merge across per-workspace result sets.
- **LLM streaming** (`src/agent/llmStreaming/`): unified SSE output for Ollama, OpenAI, Anthropic, OpenRouter, and Gemini providers (plus any OpenAI-compatible endpoint via the `openai` adapter), with per-provider parsers and reasoning-format support (ollama-thinking, deepseek-tag, gpt-oss-harmony, openai-reasoning, anthropic-thinking-delta). SSE events: `token`, `thinking`, `status`, `citations`, `done`, `error`. The `done` event carries `chatId`, `messageId`, `iterations`, `tokenUsage`, `model`, `providerType`, `mcpSources`, `resolvedWikilinks`, `doneReason`, `pipeline`, and `dlp_matches` (when DLP is enabled and matches were found).
- **Model resolution priority**: per-chat override → workspace default (`WorkspaceAgentConfig`) → global default provider (`Provider.isDefault`) → environment variables (`LLM_PROVIDER`, `LLM_MODEL`). Provider/model availability is refreshed at startup (`refreshModels` for Ollama providers) and on demand (`POST /api/providers/:id/models/refresh`).
...
- **Graceful degradation**: `getLicenseInfo()` checks expiration at runtime. An expired Enterprise license automatically reverts to Community tier — no restart needed.
...
- **Distributed locks** (`src/services/distributedLock.ts`): redlock 5.0.0-beta.2 singleton with `retryCount: 0`. `withDistributedLock(resource, durationMs, routine)` returns `null` on contention (skip) and runs locally when Redis is absent; `acquireRedisLock`/`releaseRedisLock` fall through to the PostgreSQL mutex. `cleanStaleLocks()` reaps abandoned lock keys.
...
`src/services/jobQueue.ts` manages the `PgBoss` singleton (own `pg.Pool`, auto-creates the `pgboss` schema). `start()` failure degrades to `getBoss() === null` — never throws, never exits. Cron jobs (registered at boot, `NODE_ENV=production`):

| Queue | Schedule | Purpose |
|-------|----------|---------|
| `mcp:healthcheck` | every 30 min | Marketplace MCP server health checks |
| `mcp:reaper` | every 5 min | Probes `listTools` on enabled connections, disconnects stale ones |
| `synthesis:reaper` | every 15 min | Flips orphaned `PROCESSING` synthesis runs to `FAILED` |
| `fidelity:sampling` | weekly Sun 03:00 UTC | Synthesis output quality sampling |
| `vector:cleanup` | — | Orphaned vector entry cleanup |
| `upload-draft:reaper` | daily 03:00 UTC | Soft-deletes expired UploadDrafts + best-effort unlink |
| `chat-message:reaper` | daily 03:00 UTC | Two-pass soft/hard chat-message retention purge (7-day grace, audit logged) |
| `wiki:consistency` | — | Archive wiki graph drift detection + reindex |
...
- **OCR pipeline** (`src/services/ocrJobService.ts`): job lifecycle (pending → running → awaiting_review → approved/rejected), model catalog, per-user preferences. New OCR models addable via configuration without code changes.
...
- **Push notifications** (VAPID Web Push via `web-push`, license-gated), **webhooks** (HMAC-SHA256, exponential backoff retry, auto-disable after 10 consecutive failures), **OpenAPI docs** at `/api-docs` (swagger-jsdoc 6.x + swagger-ui-express 5.x), **Winston** structured logging to `storage/logs/`.
