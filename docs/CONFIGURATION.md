...
| `OCR_NUM_CTX` | No | `0` | Runtime num_ctx override for vision OCR; `0` = registry fallback (`contextWindow`: glm-ocr 4096, glm-ocr-optimized 16384); `> 0` wins. Trained GGUF cap audited at runtime via `client.show()` (silent-clamp warn — see [`docs/OCR_RELIABILITY.md`](OCR_RELIABILITY.md)). |
...
**Vision OCR runbook (Phase 205).** `glm-ocr-optimized` is recommended for
reliability (deterministic sampling + wide context): create it with
`OLLAMA_BASE_URL=... node scripts/build-glm-ocr-optimized.cjs`, or — inside
a containerized daemon without node — `docker exec <ollama-container> ollama
create glm-ocr-optimized -f - < docker/ollama/glm-ocr-optimized.Modelfile`.
Host-downloaded models do **not** appear in the container: the Ollama
daemon's model store is the only relevant runtime state (full runbook +
measured reliability tables in [`docs/OCR_RELIABILITY.md`](OCR_RELIABILITY.md)).
The standardized OCR prompt is admin-editable at runtime via the `OCR_PROMPT`
SystemConfig key (default `Text recognition:`; `PUT /api/system/settings`, or
plain `OCR_PROMPT=` in the env — both work; the key is not ENV-only).
...
See [docs/MODEL_COSTS.md](MODEL_COSTS.md) — per-model per-token pricing via the Plugin/Providers settings UI (Phase 203).
