...
- **Managed plugin storage**: installed plugin packages live under `storage/plugins/<slug>` inside the `server-storage` volume — include it in your persistence/backup strategy; uninstalling a plugin removes its directory there (see [docs/PLUGINS.md](PLUGINS.md)).
