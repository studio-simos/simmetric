...
- **Canonical error envelope** (api-design sweep, 2026-09-24): errors return `{ "error": { "code", "message", "details"?, "requestId" } }` — `code` is machine-readable, stable, snake_case; `message` is human-readable and may change; `details` carries structured context (Zod field errors, ids); `requestId` is the per-request correlation id (also echoed as the `X-Request-Id` response header — pass it when reporting bugs). New and migrated endpoints use the envelope via `utils/httpError.ts sendError()`:

  ```json
  { "error": { "code": "document_not_found", "message": "Document not found", "requestId": "b1c2…" } }
  ```

  Legacy endpoints may still answer `{ "error": "<prose>" }` (optionally with Zod `details`); the frontend reader accepts both shapes while the migration completes. Legacy sites flip to the envelope as they are touched.
- **Error codes** (stable, machine-readable): `invalid_query`, `invalid_body`, `invalid_cursor`, `not_found`, `internal_error`, `document_not_found`, `document_not_completed`, `document_scanning`, `dlp_placeholder_edit`, `stale_version`, `access_denied` — extended as endpoints migrate; a code must be added here before it ships.
- **Request id**: every response carries `X-Request-Id`; clients may send their own `X-Request-Id` (≤128 chars) for end-to-end correlation.
...
- **404**: unknown routes return the `not_found` envelope; missing resources return domain codes (e.g. `document_not_found`).
- **500**: unhandled errors are logged server-side (with the requestId) and return the generic `internal_error` envelope — internal error text never reaches clients.
- **Optimistic concurrency**: state-changing PUTs MAY accept `If-Match: "<updatedAt>"` (the validator every serialized row carries). Absent header = check skipped (non-breaking); mismatch = `409 { error: { code: "stale_version", …, "details": { "currentUpdatedAt": "…" } } }` so the caller can re-read and retry. `GET /api/documents` rows now carry `updatedAt` (additive).
...
Cursor-based, opt-in, additive (api-design sweep, 2026-09-24):

- Collection endpoints that support it accept `?limit=<1..100>` and `?cursor=<opaque>` (default limit 50; hard cap 100 — there is no "return everything" mode once a page size is requested).
- **Cursor mode**: passing `limit` or `cursor` switches the response to `{ "items": [...], "nextCursor": "<opaque>|null" }`. `nextCursor` is `null` on the last page; feed it back as `?cursor=` to get the next page.
- **Legacy mode**: omitting both params keeps the historical bare-array response byte-identical — existing consumers keep working until they migrate.
- Cursors are opaque base64url strings (`v1|<timestamp>|<id>`); treat them as opaque, never parse them client-side. Keyset ordering is `(timestamp DESC, id DESC)` with an id tiebreaker, so pages are stable under concurrent writes.
- Live today: `GET /api/documents`, `GET /api/workspaces/:workspaceId/chats`.
- New collection endpoints MUST ship cursor pagination from day one (offset pagination is acceptable only for small, admin-facing tables with explicit jump-to-page needs).

## Evolution and versioning

**Stance: additive-only.** New optional request fields, new response fields, new endpoints, and new optional query parameters are non-breaking and ship without version bumps (precedents: `dlpEntityCount` on the document list rows, `connectorPlatform` on chat rows, `updatedAt` on document rows).

- **Breaking changes** (removing/renaming fields, changing types or semantics, tightening validation) will introduce a path version (`/api/v2/`) with the previous version kept under a documented sunset window — never in-place.
- **Deprecations** are announced in this page and via a `Sunset`/`Deprecation` response header on the affected routes.
- **The contract lives in the repo**: route-file OpenAPI annotations (surfaced at `/api-docs`) plus this page. A PR that changes an endpoint updates both.
- **Namespace note**: the widget embed service (port 3211) owns `/api/sessions`, `/api/config`, `/api/chat`, `/api/lead`; the main API owns the rest of `/api`. The two are disambiguated by port/proxy routing, not by version — keep it that way until a public third-party contract forces a version prefix.
...
| PUT | `/api/documents/:documentId/text` | Auth (`document:write` + workspace access triple-check, no admin bypass) | Body-only text edit: `{ body: string }` (min 1) → `202 { documentId, status: "reindexing" }`; the stored source is rewritten through the storage provider (text types overwrite in place, binary types swap to a `.txt` source) and the document is re-dispatched to the collector for a full re-index (async completion via the status callback). A DLP-scanned document clears its scan markers on edit so the edited text is re-scanned/re-masked on completion. Optimistic concurrency: optional `If-Match: "<updatedAt>"` — mismatch → `409 stale_version` with `details.currentUpdatedAt`. Errors: 400 invalid body / DLP placeholder edit, 403, 404, 409 (not completed / stale version / in-flight scan) |
